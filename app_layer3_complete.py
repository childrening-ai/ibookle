...
# ================= 5. Layer 4: 雙軌搜尋 (支援 Filter) =================
def dual_track_search(query, top_k=20, exact_title_filter=None, metadata_filter=None):
    """
    支援 metadata_filter 的雙軌搜尋
    """
    candidates = {}
    
    # 1. 準備 Filter 物件 (清除 null 值)
    pinecone_filter = {}
    if metadata_filter:
        for k, v in metadata_filter.items():
            if v: # 只有非空值才加入篩選
                # 注意：Pinecone 的 filter 語法是 {"key": "value"} 或 {"key": {"$in": [...]}}
                # 這裡我們先假設是一對一精準對應
                pinecone_filter[k] = v
    ...
    if exact_title_filter:
        ...
        for doc, score in results:
            final_list.append({"doc": doc, "score": 1.0, "matched_via": ["🚀 直通車"], "is_direct_hit": True})
        return final_list

    # --- 模式 B: 語意搜尋 (加入 Filter) ---
    try:
        # 如果 filter 是空的，就傳 None 給 Pinecone，避免報錯
        final_filter = pinecone_filter if pinecone_filter else None
        
        results_shell = db_shell.similarity_search_with_score(query, k=top_k, filter=final_filter)
        results_core = db_core.similarity_search_with_score(query, k=top_k, filter=final_filter)
    except Exception as e:
        st.error(f"Pinecone Filter Error: {e}")
        return []
    ...
    return final_list # [保留完整列表] 不切斷，方便觀察
